...
@include('components.jobs.editModal')
...
<div class="mt-3 d-flex justify-content-center flex-column">
    <div class="d-flex flex-column mt-3">
        ...
        @foreach($data['data'] as $job)
            <div class="card mt-3 text" onclick="edit(id)" id="{{ $job['id'] }}" data-toggle="modal" data-target="#editModal">
                <div class="row g-0 ">
                    <div class="col-md-2">
                    ...
                    <div class="col-md-10 d-flex flex-column justify-content-between ">
                        <div class="mt-2">
                            <div class="d-flex flex-column container">
                                <div class="d-flex justify-content-between">
                                    ...
                                    @if ($job['status'] == 'Em andamento')
                                        <h5 class="text text-success"> {{ $job['status'] }}</h5>
                                    @elseif ($job['status'] == 'Pendente')
                                        <h5 class="text text-warning"> {{ $job['status'] }}</h5>
                                    @else
                                        <h5 class="text text-danger"> {{ $job['status'] }}</h5>
                                    @endif
                                </div>
                        ...
                        <div class="container d-flex justify-content-between">
                            ...
                            <h6 class="text text-secondary">{{ $job['type'] }}</h6>
                        </div>
                    </div>
